...
class TestAdresseId:
    ...
    def test_adresse_id(self, client: adressevaelger.Client):
        ...
        response_body = """{
            "status": "ok",
            "adresse": {
                "id_lokalid": "197b47fd-050a-40cb-85a9-11418e009374",
                "adressebetegnelse": "Nr. Bjertvej 87A, Nr Bjert, 6000 Kolding",
        ...
        assert adresse.id_lokalid == id
        assert adresse.adressebetegnelse == "Nr. Bjertvej 87A, Nr Bjert, 6000 Kolding"
        assert adresse.etagebetegnelse is None
        assert adresse.doerbetegnelse is None
        assert adresse.status == "3"
        assert adresse.virkningfra == datetime(
            2021, 1, 5, 14, 56, 2, 99000, tzinfo=timezone.utc
        )
        assert adresse.virkningtil is None
        assert adresse.registreringfra == datetime(
            2021, 1, 5, 14, 56, 2, 99000, tzinfo=timezone.utc
        )
        assert adresse.registreringtil is None

        husnummer = adresse.husnummer
        assert husnummer is not None
        assert husnummer.id_lokalid == "0a3f5090-48fe-32b8-e044-0003ba298018"
        ...
        assert (
            husnummer.adgangspunkt.id_lokalid == "0a3f5090-48fe-32b8-e044-0003ba298018"
        )
